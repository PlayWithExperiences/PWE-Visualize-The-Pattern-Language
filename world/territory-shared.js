...
export function composeLocal(w,ids){
 ...
 if(has(30)){w.slab(98,66,12,9,30);w.pergola(98,66,4,3,30);w.bench(106,67,30,3);w.path([[104,75],[104,62]],2,30);}
 ...
 if(has(60))for(const g of groups){w.slab(g.x+1,24,8,5,60,P.ground);w.tree(g.x+2,25,60);w.bench(g.x+3,28,60);w.path([[g.x+8,29],[g.x+16,29],[g.x+16,61]],1.2,60);}
 if(has(67))for(const g of groups){w.slab(g.x+10,g.y+8,8,17,67,P.ground);w.bench(g.x+10,g.y+9,67,2);w.path([[g.x+14,g.y+25],[g.x+14,62]],1.8,67);}
...
export function composeInstitution(w,ids){
 ...
 if(has(79)){
  ...
  for(const x of [home.x+18,home.x+22])w.planter(x,home.y+30,3,2,79);
  w.path([[home.x+7,home.y+25],[home.x+7,home.y+33]],1.6,79);w.state.residentControl={homeId,homeOrigin:[home.x,home.y],boundaryY:home.y+27};
 }
