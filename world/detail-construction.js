import {World,palette as p} from './primitives.js?v=a08df7453e28';
...
export function buildConstruction(ids){
 ...
 for(const [x,y]of corners)w.box(x,y,0,has(212)?.3:.22,has(212)?.3:.22,3.6,p.wood,has(212)?212:0,'column');
 ...
 if(has(209)){if(!has(220))w.roof(5.7,5.7,12.6,10.6,209,3.9,2);if(!has(206))w.roof(19.7,7.7,6.6,6.6,209,3.9,1);w.roof(6,16,4,2.5,209,2.7,.45);mark(209,20,15,'主厅大顶／侧室小顶／边缘低顶');}
 if(has(210)){const level=has(206)?4.85:3.8;w.slab(20,8,6,3.8,210,p.stone,level);for(const x of [20,25.7]){w.box(x,8,3.6,.3,.3,level+.12-3.6,p.wood,210,'support-extension');w.box(x,8,level+.12,.3,.3,2.5,p.wood,210,'upper-column');}w.box(20,8,level+2.62,6,.3,.24,p.wood,210,'upper-beam');mark(210,24,12,'上下层顶面与边缘支点协调');}
 ...
 w.wall(wx+ww,fy,15-wx-ww,.3,3.6);w.wall(16.4,fy,1.6,.3,3.6);w.wall(15,fy,1.4,.3,3.6-(has(224)?2.1:2.5),has(224)?224:0,(has(224)?2.1:2.5)+.12);
 ...
 if(has(226)){w.box(20,13.6,0,.7,.7,3.6,p.wood,226,'inhabited-column');w.bench(20.8,13.8,226,2.1);mark(226,21.5,15,'可倚靠的粗柱与柱边座位');}
 ...
 if(has(237)){// Door is parked open at 90 degrees so the portal remains traversable.
  w.box(15,fy,0,.1,1.3,.85,p.wood,237,'door');w.box(15,fy,.85,.1,.08,1.15,p.wood,237,'door-frame');w.box(15,fy+1.22,.85,.1,.08,1.15,p.wood,237,'door-frame');w.box(15,fy,2,.1,1.3,.12,p.wood,237,'door-frame');w.box(15.035,fy+.08,.85,.025,1.14,1.15,p.glass,237,'window');mark(237,15.7,17.5,'实心下部与透明上部 · 门扇敞开');}
 if(has(238)){for(let x=wx;x<wx+ww;x+=.2)w.box(x,fy-.12,top-.2,.06,.2,.5,p.wood,238,'light-filter');w.beam([wx-.2,fy-.1,top+.4],[wx+ww+.2,fy-.1,top+.4],.055,p.plant,238);mark(238,wx+.5,15,'植物与细格柔化开口上缘');}
