...
import {finishBuilding,share,note,eastWindow,rail,counter} from './building-common.js?v=f839e046c358';
export function buildEdge(ids){
 ...
 if(h(159)){w.boxes=w.boxes.filter(b=>!(b.kind==='wall'&&b.x===10&&b.y===8&&b.dx===.2&&b.dy===8));eastWindow(w,10,8,h(165)?7.8:8,159);w.chair(11,13,159);w.state.daylightOpenings=['south','west','north'];}
 ...
 if(h(160)){w.box(10,16,2.65,12,3.5,.1,p.roof,160,'ceiling');w.bench(h(163)?20.2:19,h(163)?18.6:17,160,h(163)?1.5:2);}
 if(h(161)){w.wall(10,16,.15,3.5,1.5,161);w.planter(10.4,19,3,.4,161);w.state.sunnyPlace='南侧挡风座位；日照随地点、季节而变';}
 if(h(162)){w.room(10,3,12,4,162,{roof:false,height:2.5});w.roof(9.7,2.7,12.6,4.6,162,2.62,.6);w.room(11,-.5,10,2.5,162,{roof:false,height:2.2});w.roof(10.7,-.8,10.6,3.1,162,2.32,.4);w.box(11.5,3.5,.12,3,.7,1,p.wood,162,'storage');w.state.northHeights=[4.42,3.22,2.72];}
 ...
 if(h(165)){w.boxes=w.boxes.filter(b=>!(b.y>=15.8&&b.y<16.01&&['wall','window'].includes(b.kind)));w.box(10,15.8,.12,.25,.2,2.8,p.wood,165,'post');w.box(21.75,15.8,.12,.25,.2,2.8,p.wood,165,'post');w.box(10,15.8,2.72,12,.2,.2,p.wood,165,'lintel');w.box(10.3,15.8,.12,.08,2.2,2.35,p.wood,165,'folded-panel');counter(w,19,13,165,2);w.box(18,23,.12,2,.7,.8,p.wood,165,'street-display');w.state.openFrontWidth=11.5;}
 ...
 const stairX=h(175)?30:23;w.steps(stairX,10.8,2,20,166,.16,.3);w.slab(22,16.8,stairX-20,1.8,166,p.wood,3.2);rail(w,10,h(167)?18.5:17.9,12,166,3.32);w.bench(11,16.4,166,2,3.32);share(w,[166,167],s,m);w.state.balcony={depth:h(167)?2.6:2,door:[16,15.8],floor:3.32,stair:[stairX,10.8]};}
 ...
 if(h(170)){for(const x of [3,6,9])for(const y of [27,32]){w.tree(x,y,170,.85);for(const dx of [-.45,.4])w.box(x+dx,y,2.1,.18,.18,.18,'#c58a59',170,'fruit');}w.path([[5,22],[5,25],[4.5,26],[4.5,35]],1.4,170);w.bench(8,34,170);}
 if(h(171)){for(const [x,y]of [[11,28],[14,26],[18,26],[21,28],[21,32]])w.tree(x,y,171,1.15);w.bench(13.5,28,171,1.8);w.path([[16,23],[16,29],[20,34]],1.5,171);}
 if(h(172)){for(let n=0;n<24;n++){const x=1+(n*1.73)%9,y=8+(n*2.39)%11;w.box(x<7&&y>17?x-2:x,y,0,.35+n%3*.2,.45,.12+(n%4)*.14,n%2?p.plant:'#849976',172,'wild-plant');}w.tree(4,12,172,.8);w.path([[3,22],[6,19],[8,16]],1.1,172);}
 ...
 w.planter(23,9.4,4,.7,175);w.planter(23,13.9,4,.7,175);w.path([[29,12],[29.5,12],[29.5,10],[33,10],[33,22]],1.4,175);w.state.greenhouseDoors=[[21.8,12],[29,12]];}
 ...
 if(h(177)){for(const y of [25,28,31]){const vz=h(169)?((y-25)/3)*.48:0;w.box(28,y,vz+.12,7,1.2,.15,p.soil,177,'vegetable-bed');for(let x=28.3;x<35;x+=.5)w.box(x,y+.3,vz+.27,.25,.3,.3,p.plant,177,'vegetable');}counter(w,37,30,177,1.6);w.path(h(169)?[[32,22],[26,24],[26,35],[36,35]]:[[32,22],[36,24],[36,33]],1.3,177);}
 if(h(178)){for(const [x,color,kind]of [[29,p.wood,'compost-input'],[31.5,p.stone,'managed-treatment'],[34,p.soil,'mature-compost']]){w.box(x,37,0,1.7,1.5,.9,color,178,kind);w.box(x,37,.9,1.7,1.5,.1,p.wood,178,'lid');}w.path([[29.8,36.5],[39.2,36.5],[39.2,22],[37,22]],1.2,178);w.state.compostCycle={stages:['分类有机物','需专业管理的处理','成熟物料供种植'],sanitationVerified:false,rawWasteToCrops:false};}
 const positions={159:[11,12],160:[12,17],161:[11,18],162:[16,5],163:[19,18],164:[18,15],165:[16,16],166:[h(175)?30:23,11],167:[13,17],168:[16,20],169:[25,25],170:[6,29],171:[16,29],172:[6,16],173:[5,24],174:[16,24],175:[26,12],176:[36,12],177:[32,28],178:[32,37]};
 ...
 // Keep a full pedestrian gap between the stepped northern volumes.
 if(h(162)){
  w.routes.push({pattern:162,z:0,width:1,points:[[33,22],[33,7.5],[16,7.5],[16,6]]});
  w.routes.push({pattern:162,z:0,width:1,points:[[23,7.5],[23,2.5],[16,2.5],[16,1]]});
 }
 if(any(166,167)){const x=(h(175)?30:23)+1;w.routes.push({pattern:166,z:0,width:1.2,points:[[x,10.3],[x,17.5],[16,17.5],[16,14.5]]});}
 if(h(175))w.routes.push({pattern:175,z:.12,width:1.2,points:[[21,12],[29,12]]});
