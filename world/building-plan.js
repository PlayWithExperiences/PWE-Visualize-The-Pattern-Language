...
import {finishBuilding,share,note,portal,eastWindow,counter,bookcase,rail} from './building-common.js?v=f839e046c358';
export function buildPlan(ids){
 ...
 if(h(127)){w.path([[22,30],[22,23],[25,23],[25,15.6],[14,15.6]],1.4,127,p.wood,.13);w.wall(16.8,20,.15,4,1.4,127);w.state.privacyGradient=['入口','共享起居','侧向过道','私人床位'];}
 ...
 if(h(129))w.path([[22,29],[22,23],[25,23],[25,15.6]],1.3,129,p.stone,.13);
 ...
 if(h(131)){portal(w,11,24,10,131);portal(w,23,24,10,131);w.path([[16,25],[16,23],[16.4,23],[16.4,18.3],[28.4,18.3],[28.4,25]],1.4,131,p.wood,.13);w.state.sharedRoomLoop=[[16,25],[16,23],[16.4,23],[16.4,18.3],[28.4,18.3],[28.4,25]];}
 if(h(132)){w.wall(11,17,7,.15,1.1,132);w.bench(11.5,16,132);bookcase(w,15,16,132);w.path([[14,15.6],[18,15.6],[25,15.6]],1.2,132,p.stone,.13);}
 ...
 if(h(137)){if(!h(143))w.box(21,12,.15,1.4,1.4,.06,p.warm,137,'play-mat');w.path([[22,14.5],[22,15.6],[25,15.6],[25,18],[33,18],[33,19],[36,19],[36,21.5]],1.4,137);w.box(35,22,.15,2,2,.3,p.soil,137,'sand-play');}
 ...
 if(any(146,148,151,152)){const s=w.boxes.length,m=w.meshes.length;w.room(1,9,7,7,146,{roof:false,sideDoor:true});w.roof(.7,8.7,7.6,7.6,146,2.92,.8);w.path([[7.5,12.5],[9,12.5],[9,31],[15,31],[15,34],[22,34],[22,29]],1.5,146);share(w,[146,148,151,152],s,m);}
 ...
 if(h(150)){w.chair(12,22,150,1);bookcase(w,11,21,150);w.table(29,24.7,150,1.4,.6);w.chair(30,25,150);}
 if(h(151)){w.room(1,19,7,6,151,{roof:false});w.table(3,21,151,2,1);for(const x of [2,5.5])w.chair(x,21,151);w.path([[4.5,24.5],[4.5,31],[15,31],[15,34],[22,34],[22,32]],1.4,151);}
 if(h(152)){w.wall(5,10,.13,2,1.7,152);w.table(5.6,10,152,1.4,.65);w.chair(5.8,11,152);w.chair(6,14.7,152);}
 if(any(153,154,155)){for(const [id,y]of [[153,2],[154,12],[155,23]])if(h(id)){w.room(39,y,7,7,id,{roof:false,sideDoor:true});w.roof(38.7,y-.3,7.6,7.6,id,2.92,1);w.bed(39.7,y+1,id,1.1);counter(w,43.5,y+1,id,1.6);w.box(44.5,y+2.5,.12,.6,.7,.4,p.stone,id,'toilet');w.path([[42.5,y+6.5],[42.5,y+8],[47,y+8],[47,31.5],[38.5,31.5],[38.5,30.31],[33,30.31],[33,31.8],[22,31.8]],1.4,id);}}
 if(h(153)){
 ...
 if(h(154)){w.table(42.5,16,154,1.8,.6);w.path([[42.5,20],[38.5,20],[38.5,30.31],[33,30.31],[33,31.8],[22,31.8],[22,32]],1.3,154);}
 if(h(155)){w.bench(39.5,30.5,155,2);w.path([[42.5,30],[42.5,34]],2,155);}
 if(any(156,157)){const s=w.boxes.length,m=w.meshes.length;w.room(10,35,8,6,156,{roof:false});w.roof(9.7,34.7,8.6,6.6,156,2.92,1);counter(w,10.5,35.5,156,5);w.chair(13,37,156);w.path([[14,40.5],[14,42],[20,42],[22,34],[22,33]],1.6,156);share(w,[156,157],s,m);}
 ...
 // Validate arrival, ascent and the upper room portal as one continuous walk.
 if(any(133,158))w.routes.push({pattern:133,z:0,width:1.2,points:[[36,30],[36,37.8],[34.6,37.8],[34.6,37],[32.8,37]]});
