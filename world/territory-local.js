...
export function buildLocalPattern(ctx){const {w,id,x,y,box,path,room,house,tree,bench,table,pergola,slab,gate,station,garden}=kit(ctx);
 ...
 case 46: pergola(1,1,9,23);for(const b of [2,9,16]){table(2,b,3);box(6,b,0,2,2,1,P.wood,'market-stall');}path([[8,1],[8,26]],2);room(19,1,8,6);break;
 ...
 case 58: path([[5,0],[5,27]],3);for(const b of [1,9,17]){pergola(19,b,7,5);table(20,b+1);}slab(1,2,9,6,P.wood,.24);for(let n=0;n<2;n++)box(2,8+n*.35,0,3,.35,(2-n)*.12,P.stone,'step');break;
 case 59: room(1,1,9,9);path([[5,12],[5,22],[9,24]],1.3);garden(1,15,9,11);w.wall(x+1,y+13,9,.3,2.5,id);break;
 ...
 case 66: for(const [b,span]of [[24,8],[16,6],[8,4]]){gate(2,b,span);w.wall(x+1,y+b,.2,5,2,id);w.wall(x+10,y+b,.2,5,2,id);}box(4,2,0,3,3,1,P.stone,'inner-core');path([[6,0],[6,28]],2);break;
