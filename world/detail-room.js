...
export function buildRooms(ids){
 ...
 if(has(179)){
  w.wall(2.3,12,3,.18,2.4,179);w.wall(5.1,12,.18,1.3,2.4,179);w.box(2.3,12,2.5,3,2.7,.12,p.wood,179,'ceiling');w.bench(2.6,13.3,179,2.1);mark(179,4,14.4,'主室边缘的小凹室');
 }
 if(has(180)||has(202)){
 ...
 if(has(182)){w.box(16,14,2.5,3,1.15,.12,p.warm,182,'lamp');w.box(19.8,14,0,.45,1.5,1,p.wood,182,'sideboard');mark(182,17.5,15.4,'餐桌聚集 · 四周保留拉椅空间');}
 if(has(183)){w.table(20,5,183,2,1);chairFacing(w,20.5,6.6,183,0,2);w.wall(19.4,4.6,.18,3.2,1.3,183);w.wall(19.4,7.6,3,.18,1.3,183);mark(183,21,7,'有背靠且朝向室内的工作位');}
 if(has(184)||has(199)){
 ...
 if(has(187)||has(188)){
  ...
  if(has(188)){w.wall(3.3,3.3,3.4,.18,2.4,188);w.wall(3.3,3.3,.18,3.6,2.4,188);w.wall(6.5,3.3,.18,2.5,2.4,188);w.box(3.3,3.3,2.5,3.4,3.6,.1,p.wall,188,'ceiling');w.box(6,3.6,.12,.35,1.2,1.2,p.wood,188,'bed-storage');mark(188,6.3,6.7,'小床凹室连着更大的活动室');}
 }
 ...
 if(has(191)){for(const x of [3,7,11])w.beam([x,11,3.5],[x,19,3.5],.055,p.wood,191);w.box(3,11,.125,8,8,.025,'#e3d1b2',191,'floor');mark(191,10.5,18,'矩形主室与对称顶面');}
 if(has(192)){w.path([[3,22],[25,22]],1.3,192);w.tree(5,23,192,.8);w.bench(21,22.8,192);mark(192,20,20.8,'窗口看见花园路径和停留活动');}
 if(has(193)){w.wall(11,10,.22,3,1,193);for(const y of [10,13])w.box(11,y,1,.22,.22,2,p.wood,193,'post');w.box(11,10,3,.22,3.2,.15,p.wood,193,'beam');mark(193,12,11,'半墙和柱保留联系与独立');}
 if(has(194)){
 ...
 if(has(197)){for(const x of [3,4,5]){w.box(x,2.25,.12,.08,.5,2,p.wood,197,'storage-divider');for(const z of [.6,1.2,1.8])w.box(x,2.25,z,.9,.5,.07,p.wood,197,'shelf');}mark(197,4,3,'外墙厚度容纳储物与生活');}
 ...
 if(has(203)){w.wall(17.5,3,.12,1.6,1.05,203);w.wall(19,3,.12,1.6,1.05,203);w.wall(17.5,3,1.62,.12,1.05,203);w.box(17.5,3,1.17,1.62,1.6,.12,p.wood,203,'ceiling');w.box(17.7,3.2,.12,1.2,1.1,.05,p.warm,203,'floor');mark(203,18.3,5,'儿童尺度凹洞 · 照护与疏散另评');}
 if(has(204)){w.box(7,2.3,.9,.7,.5,.06,p.wood,204,'memory-niche');w.box(7,2.3,.9,.06,.5,.65,p.wood,204);w.box(7.64,2.3,.9,.06,.5,.65,p.wood,204);w.box(7.1,2.4,.98,.4,.25,.25,p.warm,204,'memory-box');mark(204,7.3,3.2,'边缘里的私人记忆藏处');}
