...
import {finishBuilding,share,note,car,rail} from './building-common.js?v=f839e046c358';
export function buildSite(ids){
 ...
 if(h(98)){for(const [x,y,ww]of [[h(112)?20:21,32,h(112)?9:6],[22,25,4],[23,h(109)&&entryRoof?entryY+entryDepth+.5:20,2]]){w.box(x,y,0,.15,.15,2.6,p.wood,98);w.box(x+ww,y,0,.15,.15,2.6,p.wood,98);w.box(x,y,2.6,ww,.15,.16,p.wood,98);}w.state.arrivalRealms=['公共到达','院落','主入口'];}
 ...
 if(h(101)){w.path([[24,6],[24,md+9]],2.4,101);w.bench(mx+.5,10,101);w.box(mx+mw-1,9,0,.5,3,.8,p.wood,101,'display');}
 if(h(104)){w.tree(5,22,104,1.3);w.path([[8,20],[14,20],[14,26],[12,26]],1.4,104);w.box(36,3,0,8,3,.04,p.soil,104,'repair-ground');w.state.siteRepair={retainedTree:[5,22],builtOnLessValuedGround:[24,11]};}
 ...
 if(h(111)){const x=h(118)?46:32;w.wall(x,23,.2,7,1.7,111);w.planter(x+2,23,7,.7,111);w.bench(x+3,26,111);w.path([[24,30],[24,32.5],[34,32.5],[34,33.3],[x+1.5,33.3],[x+3,31],[x+3,28]],1.3,111);}
 ...
 if(h(114)){w.wall(8,23,3,.2,1,114);w.wall(8,23,.2,3,1,114);w.bench(8.4,23.4,114);w.path([[10,26],[12,26],[12,30],[24,30],[24,33]],1.4,114);}
 if(h(115)){w.bench(10.5,21.5,115,2);w.path([[8,16],[8,entryY+1.1],[24,entryY+1.1],[40,entryY+1.1],[40,16]],2.2,115);}
 if(h(116))w.state.roofHierarchy=[mainH+.12+(h(117)?2.1:1.2),3.77,2.64+(h(117)?.8:.45)];
 if(h(117))w.bench(entryX+.5,entryY+.1,117);
 ...
 if(h(120)){w.tree(13,31,120);w.bench(15,31.3,120);w.path([[24,35],[15,34],[13,33]],1.5,120);}
 ...
 if(h(126)){const x=h(112)?30:28,y=h(112)?33.2:32;w.tree(x,y,126,1.2);w.bench(x+1.5,h(112)?34.3:31,126);w.path([[24,35],[24,30]],2,126);}
 const positions={95:[8,16],96:[24,5],97:[6,28],98:[24,32],99:[24,md+7],100:[15,20],101:[24,10],102:[8,16],103:[40,33],104:[5,22],105:[23,26],106:[16,25],107:[mx,11],108:[14,17],109:[24,11],110:[24,md+8],111:[h(118)?49:35,27],112:[27,31],113:[14,33],114:[10,25],115:[24,20],116:[24,md+7],117:[mx+1,md+8],118:[45,17],119:[30,entryY+1.1],120:[13,31],121:[20,27],122:[8,16],123:[6,36],124:[4,22],125:[20,36],126:[h(112)?30:28,h(112)?33.2:32]};
 ...
 if(h(96))w.routes.push({pattern:96,width:.6,z:0,endFeet:3.32,points:[[32,-4.4],[32,2.4],[29,2.5]]});
 if(h(118))w.routes.push({pattern:118,width:.6,z:0,endFeet:3,points:[[46,16],[46,22.2],[43.5,22.2],[42,24]]});
