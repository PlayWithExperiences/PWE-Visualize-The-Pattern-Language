import {defaultSun,formatHour} from './sun.js?v=a08df7453e28';
import {groups,groupFor,compose,normalizeIds,toggleId,encodeAtlas,decodeAtlas,alternatives,influences} from './atlas/compose.js?v=a08df7453e28';
import {esc} from './atlas/primitives.js?v=a08df7453e28';
...
async function init(){
 try{[guides,relations]=await Promise.all(['./data/guide-catalog.json?v=a08df7453e28','./data/book-relations.json?v=a08df7453e28'].map(async url=>{const r=await fetch(url);if(!r.ok)throw Error(`${url}: HTTP ${r.status}`);return r.json();}));if(guides.length!==253)throw Error('模式数据不完整');}catch(error){status('资料载入失败：'+error.message+'。请刷新重试。');$('#drawing').textContent='无法加载模式资料。';return;}
