import {groups,groupFor,compose,normalizeIds,toggleId,encodeAtlas,decodeAtlas,alternatives,influences} from './atlas/compose.js';
import {esc} from './atlas/primitives.js';
...
let guides,relations,saved=null,state=decodeAtlas(location.hash)||{group:'site',ids:[104,105,106,110,112,115,119,120,125,126],focus:115,view:'combined'};
...
function renderDetail(){
 ...
 $('#detail').innerHTML=`<div><span class="number">${String(p.id).padStart(3,'0')}</span><h3>${esc(p.zh)}</h3><p class="english">${esc(p.name)}</p><button class="toggle ${state.ids.includes(p.id)?'':'primary'}" data-toggle="${p.id}">${state.ids.includes(p.id)?'从方案移除 −':'加入我的方案 +'}</button></div><div><h4>它要解决什么</h4><p>${esc(p.problem)}</p><h4>原书原则 · 项目概括</h4><p>${esc(p.principle)}</p>${p.note?`<p class="note">${esc(p.note)}</p>`:''}<h4>在这里观察</h4><p>${esc(groupFor(p.id).description)}。可切换“单项图解”观察本项，再回到组合中比较；橙色轮廓标出本项参与的图形。</p></div><div class="source"><strong>来源定位 · A Pattern Language (1977)</strong><p>扫描本 PDF 第 ${p.source.pdfPages.join('、')} 页<br>候选印刷页 ${p.source.printedStartCandidate}–${p.source.printedEndCandidate}${p.source.solutionPdfPage?' · 方案段 PDF 第 '+p.source.solutionPdfPage+' 页':''}</p><p>依据：${p.source.reviewBasis==='solution-section'?'方案段及相关正文':'现有正文（详见本项说明）'}。页界由文字页码恢复，非逐页校勘。图解是项目解释。</p><details><summary>原书提及 ${out.length} 项 / 被提及 ${incoming.length} 项</summary><p>匹配英文名与编号的有向引用；不是强制依赖，亦非完整关系网。</p><div class="relations">${buttons(out)}</div>${incoming.length?`<details><summary>查看哪些模式提及本项</summary><div class="relations">${buttons(incoming)}</div></details>`:''}</details></div>`;
}
function render(){
...
function loadSaved(){try{const raw=localStorage.getItem(storageKey);if(!raw)return null;const s=decodeAtlas('#atlas='+encodeURIComponent(raw));if(!s)throw Error('保存格式无法读取');return s;}catch(error){status('无法读取本机方案：'+error.message);return null;}}
async function init(){
 try{[guides,relations]=await Promise.all(['./data/guide-catalog.json','./data/book-relations.json'].map(async url=>{const r=await fetch(url);if(!r.ok)throw Error(`${url}: HTTP ${r.status}`);return r.json();}));if(guides.length!==253)throw Error('模式数据不完整');}catch(error){status('资料载入失败：'+error.message+'。请刷新重试。');$('#drawing').textContent='无法加载模式资料。';return;}
 document.addEventListener('click',e=>{const t=e.target.closest('button');if(!t)return;if(t.dataset.focus)select(+t.dataset.focus);if(t.dataset.toggle)toggle(+t.dataset.toggle);if(t.dataset.group){state.group=t.dataset.group;state.focus=groups.find(g=>g.key===state.group).from;render();}if(t.classList.contains('close'))t.closest('dialog').close();});
 ...
 $('#apply-preset').onclick=()=>{const ids=presets[state.group][+$('#preset').value][1];state.ids=normalizeIds([...state.ids.filter(id=>groupFor(id).key!==state.group),...ids]);state.focus=ids[0];state.view='combined';render();status('已应用本尺度的示例组合。');};
 ...
 $('#share').onclick=async()=>{const url=location.href;try{await navigator.clipboard.writeText(url);status('组合链接已复制。');}catch{$('#share-url').hidden=false;$('#share-url').value=url;$('#share-url').select();status('请复制下方完整链接。');}};
 ...
 $('#compare').onclick=()=>{saved=loadSaved();if(!saved){status('请先保存一个本机方案，再进行比较。');return;}$('#comparison').innerHTML=[['当前组合',state],['保存的组合',saved]].map(([label,s])=>`<section><h3>${label}</h3>${compose(state.group,s.ids).svg}<p>本尺度：${s.ids.filter(id=>groupFor(id).key===state.group).join('、')||'未选模式'}</p></section>`).join('');$('#compare-dialog').showModal();};
