import {defaultSun,formatHour} from './sun.js?v=3534622f64ec';
import {groups,groupFor,compose,normalizeIds,toggleId,encodeAtlas,decodeAtlas,alternatives,influences} from './atlas/compose.js?v=3534622f64ec';
import {esc} from './atlas/primitives.js?v=3534622f64ec';
...
async function renderWorld(){
 ...
 try{if(!worldViewer){worldImport??=import('./world/viewer.js?v=3534622f64ec');const {createWorldViewer}=await worldImport;if(request!==worldRequest)return;worldViewer=createWorldViewer({onSun:(sun,save)=>{state.sun=sun;if(save)persistURL();},onCamera:camera=>{state.camera=camera;render();},onSelect:select,onTitle:id=>guide(id)?.zh,onCutaway:value=>{state.cutaway=value;render();},onError:status});}
...
async function init(){
 try{[guides,relations]=await Promise.all(['./data/guide-catalog.json?v=3534622f64ec','./data/book-relations.json?v=3534622f64ec'].map(async url=>{const r=await fetch(url);if(!r.ok)throw Error(`${url}: HTTP ${r.status}`);return r.json();}));if(guides.length!==253)throw Error('模式数据不完整');}catch(error){status('资料载入失败：'+error.message+'。请刷新重试。');$('#drawing').textContent='无法加载模式资料。';return;}
