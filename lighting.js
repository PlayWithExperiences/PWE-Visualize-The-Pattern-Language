...
export function sunMatrix(scene){
 const base=scene.boxes.find(b=>b.kind==='ground');
 const center=[base.x+base.dx/2,1,base.y+base.dy/2];
 const length=Math.hypot(...SUN_DIRECTION),back=SUN_DIRECTION.map(x=>x/length);
 const eye=center.map((x,i)=>x+back[i]*40);
 const rightLength=Math.hypot(back[2],back[0]),right=[back[2]/rightLength,0,-back[0]/rightLength];
...
void main(){
 ...
 float sunlight=visibility(normal)*max(dot(normal,uSunDirection),0.0);
 float hemisphere=normal.y*.5+.5;
 vec3 ambient=mix(vec3(.26,.235,.20),vec3(.42,.47,.52),hemisphere);
 vec3 radiance=albedo*(ambient+vec3(1.75,1.55,1.25)*sunlight);
 ...
 if(vMaterial>2.5&&vMaterial<3.5){
  ...
  radiance=mix(albedo*.5,vec3(.45,.62,.74),.35+fresnel*.5)+sparkle*.5;
  alpha=.10+fresnel*.30;
 }
 vec3 color=pow(aces(radiance),vec3(1.0/2.2));
 float fog=clamp((vDistance-22.0)/80.0,0.0,.3);
 gl_FragColor=vec4(mix(color,vec3(.77,.84,.87),fog),alpha);
}`;
export function createSunlight(gl,compile){
 ...
 return {texture,size,matrix:null,
  update(scene,vertices){
   this.matrix=sunMatrix(scene);
